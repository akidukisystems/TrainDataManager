 ...
 * - TIME TABLE: arrive/ depart/ enter(=arriveSpeedLimit)/ exit(=departSpeedLimit)/ track 取り込み
 * - trackId=-1 は「-」、1～9 は表がなくても番号をそのまま表示
 */
public class DutyCardReader {

    // ---------- モデル ----------
    public static class Line { public int id; public String name; }
    public static class Station { public int id; public String name; public int lineId; public double linePost; }
    public static class TrainType { public int id; public String nameJa; public String nameEn; }
    public static class Track { public int id; public String name; }
    ...
    // ---------- セクション ----------
    private enum Section { NONE, LINE, STATION, TRAIN_TYPE, TRACK, STOP_PATTERN, TIME_TABLE, TRAIN_NUMBER }
    ...
    public static DutyCardData parse(List<String> lines) {
        ...
        for (String raw : lines) {
            ...
            if (maybe != Section.NONE) {
                section = maybe; headers = null; headerSeen = false;
                continue;
            }
            if (section == Section.NONE) continue;

            if (!headerSeen) { headers = splitKeepEmpty(raw); headerSeen = true; continue; }
            ...
            switch (section) {
                case LINE -> {
                    ...
                    if (id >= 0 && name != null) {
                        Line x = new Line(); x.id = id; x.name = name;
                        data.lines.put(id, x);
                    }
                }
                case STATION -> {
                    ...
                    if (id >= 0 && name != null) {
                        ...
                        s.id = id; s.name = name; s.lineId = lineId; s.linePost = post;
                ...
                case TRAIN_TYPE -> {
                    ...
                    if (id >= 0) {
                        ...
                        t.id = id; t.nameJa = ja; t.nameEn = en;
                ...
                case TRACK -> {
                    ...
                    if (id >= 0 && name != null) {
                        Track t = new Track(); t.id = id; t.name = name;
                ...
                case STOP_PATTERN -> {
                    ...
                    StopPattern sp = data.stopPatterns.computeIfAbsent(id, k -> {
                        StopPattern x = new StopPattern(); x.id = k; return x;
                    });

                    // memo はパターン全体の説明。どの行に書いてあっても拾う
                    String memo = getStr(cols, idx(headers, "memo"));
                    if (memo != null && (sp.memo == null || sp.memo.isEmpty())) sp.memo = memo;
                ...
                case TIME_TABLE -> {
                    // ヘッダ: index,stopStationCode,arriveTime,departTime,arriveSpeedLimit,departSpeedLimit,track,
                    int id = getInt(cols, idx(headers, "index"), -1);
                    int st = getInt(cols, idx(headers, "stopStationCode"), -1);
                    if (id < 0 || st < 0) break;
                    ...
                    TimeTable tt = data.timeTables.computeIfAbsent(id, k -> {
                        TimeTable x = new TimeTable(); x.id = k; return x;
                    });
    ...
    public static String lineName(DutyCardData d, int id) {
        Line l = d.lines.get(id); return l != null ? l.name : "#" + id;
    }
    public static String stationName(DutyCardData d, int id) {
        Station s = d.stations.get(id); return s != null ? s.name : "#" + id;
    }
    public static String stationLineName(DutyCardData d, int id) {
        Station s = d.stations.get(id); return s != null ? lineName(d, s.lineId) : "?";
    }
    ...
    public static String trackName(DutyCardData d, int id) {
        ...
        if (t != null && t.name != null && !t.name.isEmpty()) return t.name;
        if (1 <= id && id <= 9) return String.valueOf(id);
        return "#" + id;
    }
    ...
    public static List<String> resolveStopPatternPretty(DutyCardData d, int id) {
        ...
        if (sp == null) return List.of("(stopPattern " + id + " not found)");
        List<String> out = new ArrayList<>();
        if (sp.memo != null && !sp.memo.isEmpty()) out.add("〈" + sp.memo + "〉");
        int i = 1;
    ...
    public static List<String> resolveTimeTablePretty(DutyCardData d, int id) {
        ...
        if (tt == null) return List.of("(timeTable " + id + " not found)");
        List<String> out = new ArrayList<>();
        int i = 1;
    ...
    public static String resolveTrainNumberPretty(DutyCardData d, int id) {
        ...
        if (tn == null) return "(trainNumber " + id + " not found)";
    ...
    private static String norm(String s) {
        if (s == null) return "";
        if (!s.isEmpty() && s.charAt(0) == '\uFEFF') s = s.substring(1); // BOM除去
    ...
    private static int idx(String[] h, String n) {
        if (h == null) return -1;
        for (int i = 0; i < h.length; i++) if (n.equals(h[i])) return i;
        return -1;
    }
    private static String headerAny(String[] h, String... cands) {
        for (String n : cands) if (idx(h, n) >= 0) return n;
        return "____notfound____";
    }
    private static int getInt(String[] c, int i, int def) {
        if (i < 0 || i >= c.length) return def;
        String s = c[i].trim(); if (s.isEmpty()) return def;
    ...
    public static Direction detectDirection(DutyCardData d, TimeTable tt) {
        ...
        for (int i = 1; i < tt.entries.size(); i++) {
            ...
            if (a == b) continue;                 // 同一駅が続くケースはスキップ
        ...
        if (sign == null) return Direction.UNKNOWN; // 全て同一IDなら判定不能
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int id : d.trainNumbers.keySet()) System.out.println(resolveTrainNumberPretty(d, id));
